...
@Plugin
public class PioneerDecoder implements ProtocolDecoder {
	...
	public ProtocolInfo getInfo() {
		return new ProtocolInfo("Pioneer", "Space Length", "Pioneer", 16, 5);
	}
	...
	protected void reportPartial() {
		if (m_BitCounter > 0) {
			m_Sink.partiallyParsedMessage("Pioneer", m_BitCounter);
		}
	}
	...
	protected void addBit(int b) {
		...
		if (m_BitCounter == 31){
			...
			if ((m_Address != (m_AddressCheck ^ 0xFF)) || (m_Command != (m_CommandCheck ^ 0xFF))) {
				// Checksum error
				reportPartial();
				m_State = IDLE;
				return;
			}
			ProtocolMessage message = new ProtocolMessage("Pioneer", m_Command, m_Address, 2);
	...
	public int parse(double pulse, boolean state) {
		switch (m_State) {
			...
			case REPEAT_SCAN: {
				if (pulseCompare(pulse, 24000.0, 28000.0)) {
